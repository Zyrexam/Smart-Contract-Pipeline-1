...
"""
        ...
        # CUSTOM or any other category: keep instructions explicit so the
        # model still returns a single, compilable contract even for new
        # patterns (e.g. multisig vault with biometric oracle).
        system_prompt += """

CUSTOM-CONTRACT RULES:
- Implement exactly ONE contract in a single Solidity file.
- You MAY use OpenZeppelin helpers like Ownable and ReentrancyGuard, but
  you MUST import them correctly and ensure the code compiles standalone.
- Declare all events, errors, structs and modifiers BEFORE using them.
- Do not leave TODOs or commented-out code blocks; everything in the file
  must be valid Solidity.
- Prefer simple, explicit control flow over overly clever patterns.
"""
...
def _repair_with_model_if_needed(solidity_code: str) -> str:
    """Use a second, strict pass to fix syntax/import issues.

    This pass is category-agnostic: it should work for ERC20/721 as well as
    totally custom patterns like multisig vaults with biometric oracles.
    """

    system = (
        "You are a strict Solidity compiler and formatter. "
        "Return Solidity that compiles under ^0.8.20 with the latest "
        "OpenZeppelin contracts. Fix ONLY syntax, import paths, inheritance "
        "ordering and constructor signatures. Do NOT change high-level "
        "behaviour. If the contract inherits from Ownable, ensure the "
        "constructor calls an appropriate Ownable initializer. If imports "
        "use the NPM-style prefix '@openzeppelin/contracts/...', you may "
        "rewrite them to fully qualified GitHub URLs that Remix can compile "
        "(for example, 'https://github.com/OpenZeppelin/openzeppelin-"
        "contracts/...'). Output ONLY the corrected Solidity file, with no "
        "Markdown fences and no explanations."
    )

    user = (
        "Here is a Solidity contract that may contain syntax, import or "
        "constructor errors. Return a fixed version that compiles under "
        "Solidity ^0.8.20, preserving the same structure and intent as much "
        "as possible.\n\n" + solidity_code
    )
