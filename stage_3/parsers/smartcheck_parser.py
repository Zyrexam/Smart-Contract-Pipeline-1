"""
SmartCheck Parser - Extracted from SmartBugs tools/smartcheck/parser.py
...
# All possible SmartCheck findings (from SmartBugs)
FINDINGS = {
...
class SmartCheckParser:
    """Parse SmartCheck output using SmartBugs logic"""
    ...
    def parse(exit_code: int, log: List[str], output: bytes) -> List[SecurityIssue]:
        ...
        Args:
            ...
            output: Binary output (not used for SmartCheck)
        ...
        for line in log:
            ...
            if i >= 0:
                ...
                if k.endswith("ruleId"):
                    # Start new finding, save previous if exists
                    if finding:
